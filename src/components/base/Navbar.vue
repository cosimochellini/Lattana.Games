  ...
  <nav class="rounded-b-md transition duration-500" :class="actualState.color">
    <div class="max-w-7xl mx-auto px-2 md:px-0">
      <div class="relative flex items-center justify-between h-14 sm:h-16">
        ...
        <div
          ...
          <div class="font-semibold leading-3 text-2xl tracking-widest capitalize px-3 py-2">
            <div class="md:hidden">
              <span v-t="'navbar.route.' + actualState.name" class="transition duration-500" />
          ...
          <div class="hidden sm:block sm:ml-6">
            <div class="flex">
              <router-link v-for="(route, index) in navbarConfig.navbarRoutes" :key="route.route"
                ...
                :class="bindRouterLink(route.name, index)" v-t="'navbar.route.' + route.name" />
            </div>
          </div>
        </div>
          ...
          <div class="ml-3 relative">
            <div>
              <button
                ...
                <img @click="toggleProfile" class="h-10 w-10 rounded-full border border-gray-900" :src="profileSrc"
                  alt="User Profile" />
              </button>
            </div>
            <div class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg z-50" :class="{
              absolute: state.profileOpen,
              hidden: !state.profileOpen,
            }">
              <div class="py-1 rounded-md bg-white shadow-xs" role="menu">
                <router-link v-for="route in navbarConfig.profileRoutes" :key="route.route"
                  :to="{ name: route.route, params: { locale } }"
                  class="block px-4 py-2 capitalize text-md tracking-wider leading-5 text-gray-900 transition duration-150 ease-in-out"
                  role="menuitem" v-t="'navbar.profileRoute.' + route.name" />
                <div class="w-full mt-2">
                  <div class="inline-flex">
                    <button class="lang-button rounded-l-lg" @click="() => (currentLanguage = Languages.it)"
                      :class="{ active: currentLanguage === Languages.it }">ita</button>
                    <button class="lang-button rounded-r-lg" :class="{ active: currentLanguage === Languages.en }"
                      @click="() => (currentLanguage = Languages.en)">eng</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div :class="{
      ...
      <div class="px-2 pt-2 pb-3">
        <router-link v-for="route in navbarConfig.navbarRoutes" :key="route.route"
          :to="{ name: route.route, params: { locale } }" @click.passive="toggleNavbar"
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(router.currentRoute, (route) => {
      currentLanguage.value = route.params.locale as Languages;
    });
    ...
    return { currentLanguage, state, toggleNavbar, toggleProfile, Languages };
  },
  mounted() {
    auth.onPlayerUpdate(() => {
